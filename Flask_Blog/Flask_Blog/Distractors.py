...
education_list = ["bachelor", "master", "phd", "nobel prize"]
...
def dealing_with_education(word):
    if word not in education_list:
        return None
    output = []
    generated_before = [word]
    for iterator in range(3):  # 3 candidates
        n = random.choice(education_list)
        while n in generated_before:
            n = random.choice(education_list)
        output.append(n)
        generated_before.append(n)
                    ...
                    if output is None:
                        ...
                        if output is None:
                            ...
                            if output is None:
                                ...
                                if output is None:
                                    output = dealing_with_education(word)
    return output
...
def run_distractors(test_word, NE, w_POS):
    ...
    distractors = run_wordnet(test_word, NE, w_POS)
    multi_words = False
    multi_index = 0
    if distractors is None:
        ...
        for w in words:
            ...
            distractors = run_wordnet(w, NE, w_POS)
            if distractors:
                multi_words = True
                break
            multi_index += 1
